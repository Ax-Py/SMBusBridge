...
uint8_t display_help(){
	...
	uint8_t help[] = "I2C Dongle | XX = Hex Addr/Data | XX! = Start + Addr + W | XX? = Start + Addr + R | XX$ = Write Data Byte/ACKs | @ = Find Slave Addresses | ^ = Current I2C Bus State | T = 400kHz | S = 100kHz | V = 10kHz | % = Current Bus Frequency\n";
...
uint8_t rising_edge_pulse(uint8_t time_length){
	uint16_t ms_time_length = time_length * 100;
	
	PORTD |= (1 << PORTD7);
	
	while((ms_time_length) > 0){
		_delay_ms(1);
		ms_time_length--;
	}
	...
	return (PORTD & (1 << PORTD7)) ? INCORRECT_GPIO_CONFIGURATION : NO_ERROR;
}

uint8_t falling_edge_pulse(uint8_t time_length){
	uint16_t ms_time_length = time_length * 100;
	...
	return (!(PORTD & (1 << PORTD6))) ? INCORRECT_GPIO_CONFIGURATION : NO_ERROR;
}

uint8_t GPIO_HIGH(){
	PORTD |= (1 << PORTD5);
	
	return (!(PORTD & (1 << PORTD5))) ? INCORRECT_GPIO_CONFIGURATION : NO_ERROR;
}
...
uint8_t UART_receive_array(uint8_t I2C_status){
	...
	while ((UART_data != '\n') && (UART_receive_buffer[0] < UART_receive_buffer_length)){
		...
		switch(UART_data){
			...
			case '+':
				system_error_handler(GPIO_HIGH());
			
				special_char = 1;
				break;
			
			case '-':
				system_error_handler(GPIO_LOW());
			...
			case 'N': // Emit 100ms voltage pulse to allow for oscilloscope trigger sync
				system_error_handler(falling_edge_pulse(stacked_data));
			
				special_char = 1;
				break;
			
			case 'P': // Emit 100ms voltage pulse to allow for oscilloscope trigger sync
				system_error_handler(rising_edge_pulse(stacked_data));
